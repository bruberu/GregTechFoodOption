package gregtechfoodoption.integration.jei.multi;
...
public class NoEnergyRecipeWrapper implements IRecipeWrapper {
    ...
    private final Map<ItemStack, Recipe.ChanceEntry> chanceOutput = new Object2ObjectOpenCustomHashMap<>(strategy);
    private final List<FluidStack> notConsumedFluidInput = new ArrayList<>();

    private final Recipe recipe;

    public NoEnergyRecipeWrapper(Recipe recipe) {
        this.recipe = recipe;
    }
    ...
    public void getIngredients(IIngredients ingredients) {
        ...
        if (!recipe.getOutputs().isEmpty() || !recipe.getChancedOutputs().isEmpty()) {
            ...
            List<Recipe.ChanceEntry> chancedOutputs = recipe.getChancedOutputs();
            ...
            recipeOutputs.sort(Comparator.comparingInt(stack -> {
                Recipe.ChanceEntry chanceEntry = chanceOutput.get(stack);
                if (chanceEntry == null)
                    return 0;
    ...
    private int getPropertyListHeight() {
        return (recipe.getRecipePropertyStorage().getSize() + 2) * LINE_HEIGHT;
    }
}
