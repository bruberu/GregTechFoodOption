...
public class GTFOMetaTooltipOverlay {
    ...
    public void onRenderTooltip(RenderTooltipEvent.PostText event) {
        ...
        boolean shouldShowTooltip = (GTFOConfig.showTooltipsOnShift && KeyHelper.isShiftKeyDown()) ||
                GTFOConfig.showTooltipsAlways;
        ...
        GTFOMetaFoodHelper.BasicFoodValues modifiedFoodValues = GTFOMetaFoodHelper.getModifiedFoodValues(hoveredStack,
                player);
        ...
        float biggestSaturationIncrement = Math.max(defaultFoodValues.getSaturationIncrement(),
        ...
        String saturationText = saturationOverflow ?
                ((biggestSaturationIncrement < 0 ? -1 : 1) * saturationBarsNeeded) + "x " : null;
        ...
        int leftX = rightX - (Math.max(barsNeeded * 9 + (int) (mc.fontRenderer.getStringWidth(hungerText) * 0.75f),
                saturationBarsNeeded * 6 + (int) (mc.fontRenderer.getStringWidth(saturationText) * 0.75f))) - 3;
        ...
        Gui.drawRect(leftX, (shouldDrawBelow ? bottomY : topY - 1), rightX, (shouldDrawBelow ? bottomY + 1 : topY),
                0xF0100010);
        ...
        if (hungerText != null) {
            ...
            mc.fontRenderer.drawStringWithShadow(hungerText, x * 4 / 3 - mc.fontRenderer.getStringWidth(hungerText) + 2,
                    y * 4 / 3 + 2, 0xFFDDDDDD);
        ...
        for (int i = 0; i < saturationBarsNeeded * 2; i += 2) {
            ...
            gui.drawTexturedModalRect(x, y,
                    effectiveSaturationOfBar >= 1 ? 21 :
                            effectiveSaturationOfBar > 0.5 ? 14 :
                                    effectiveSaturationOfBar > 0.25 ? 7 : effectiveSaturationOfBar > 0 ? 0 : 28,
                    modifiedSaturationIncrement >= 0 ? 27 : 34, 7, 7);
        ...
        if (saturationText != null) {
            GlStateManager.pushMatrix();
            GlStateManager.scale(0.75F, 0.75F, 0.75F);
            mc.fontRenderer.drawStringWithShadow(saturationText,
                    x * 4 / 3 - mc.fontRenderer.getStringWidth(saturationText) + 2, y * 4 / 3 + 1, 0xFFDDDDDD);
            GlStateManager.popMatrix();
        }
