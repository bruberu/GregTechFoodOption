...
public class EntityItalianBuffalo extends EntityCow {
    ...
    private static final Biome[] NEARBY_BIOME_SPAWNS = { Biomes.OCEAN, Biomes.DEEP_OCEAN, Biomes.RIVER,
            Biomes.SWAMPLAND };
    public static final Biome[] POSSIBLE_BIOME_SPAWNS = { Biomes.SWAMPLAND, Biomes.BEACH, Biomes.COLD_BEACH,
            Biomes.FOREST, Biomes.PLAINS, Biomes.ROOFED_FOREST, Biomes.EXTREME_HILLS };
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand) {
        ...
        if (itemstack.getItem() == Items.BUCKET && this.getGrowingAge() >= 0 && !player.capabilities.isCreativeMode) {
            itemstack.shrink(1);
            ItemStack bucket = UniversalBucket.getFilledBucket(ForgeModContainer.getInstance().universalBucket,
                    GTFOMaterialHandler.ItalianBuffaloMilk.getFluid());
