...
public class GTFOEventHandler {
    ...
    @SubscribeEvent
    public static void onMaterialsInit(MaterialEvent event) { // Must be called during construction to be registered in
                                                              // time for MaterialEvents.
    ...
    public static void onLivingUpdate(LivingEvent.LivingUpdateEvent event) {
         ...
         * }
         * if (AddictionPotion.instance != null && entityLivingBase.isPotionActive(AddictionPotion.instance) &&
         ...
         * if (addictionAmplifiers.containsKey(entityLivingBase) && addictionAmplifiers.get(entityLivingBase) <
         * amplifier)
         ...
         * }
         * if (addictedSet.contains(entityLivingBase) &&
         * !event.getEntityLiving().isPotionActive(AddictionPotion.instance) && !entityLivingBase.world.isRemote) {
         * entityLivingBase.addPotionEffect(new PotionEffect(WithdrawalPotion.instance, 200,
         * addictionAmplifiers.get(entityLivingBase)));
         * addictedSet.remove(entityLivingBase);
         * addictionAmplifiers.remove(entityLivingBase);
         * }
         * if (WithdrawalPotion.instance != null && entityLivingBase.isPotionActive(WithdrawalPotion.instance)) {
         * entityLivingBase.attackEntityFrom(new DamageSource("withdrawal"), ((float)
         * (entityLivingBase.getActivePotionEffect(WithdrawalPotion.instance).getAmplifier() + 1) / 5));
         * }
         * }
         */
            ...
            if (GTFOConfig.gtfoPotionConfig.snowGolemSpawner) {
                if (SnowGolemSpawnerPotion.INSTANCE != null && player.isPotionActive(SnowGolemSpawnerPotion.INSTANCE)) {
                    if (!persisted.getBoolean(SnowGolemSpawnerPotion.TAG_NAME)) {
                        ...
                        if (!player.world.isRemote && GTFOValues.rand.nextInt(100 /
                                (player.getActivePotionEffect(SnowGolemSpawnerPotion.INSTANCE).getAmplifier() + 1)) ==
                                0) {
                            float angle = (float) (GTFOValues.rand.nextFloat() * Math.PI);

                            RayTraceResult result = player.world.rayTraceBlocks(player.getPositionVector(),
                                    new Vec3d(1, -0.3, 0).rotateYaw(angle), false, false, true);
                            EntitySnowman spawn = new EntityStrongSnowman(player.world);
                            spawn.setLocationAndAngles(result.getBlockPos().getX(), result.getBlockPos().getY() + 1,
                                    result.getBlockPos().getZ(), player.rotationYawHead, player.rotationPitch);
    ...
    // This particular function helps with generating food eating particles when the use count is over 25, because
    // Minecraft is dumb.
    ...
    public static void onLivingUpdate(LivingEntityUseItemEvent.Tick event) {
        if (event.getItem().getItem() instanceof ItemFood || event.getItem().getItem() instanceof MetaItem<?>) {
            ...
            if (livingBase.getItemInUseCount() <= GTFOFoodDurationSetter.duration(stack) - 7 &&
                    livingBase.getItemInUseCount() % 4 == 0) {
                if (stack.getItemUseAction() == EnumAction.EAT) {
                    for (int i = 0; i < 5; ++i) {
                        Vec3d vec3d = new Vec3d(((double) rand.nextFloat() - 0.5D) * 0.1D, Math.random() * 0.1D + 0.1D,
                                0.0D);
                        ...
                        vec3d1 = vec3d1.add(livingBase.posX, livingBase.posY + (double) livingBase.getEyeHeight(),
                                livingBase.posZ);
                        if (stack.getHasSubtypes()) {
                            livingBase.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec3d1.x, vec3d1.y, vec3d1.z,
                                    vec3d.x, vec3d.y + 0.05D, vec3d.z, Item.getIdFromItem(stack.getItem()),
                                    stack.getMetadata());
                        } else {
                            livingBase.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec3d1.x, vec3d1.y, vec3d1.z,
                                    vec3d.x, vec3d.y + 0.05D, vec3d.z, Item.getIdFromItem(stack.getItem()));
                    ...
                    livingBase.playSound(SoundEvents.ENTITY_GENERIC_EAT, 0.5F + 0.5F * (float) rand.nextInt(2),
                            (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
                } else if (stack.getItemUseAction() == EnumAction.DRINK) {
                    livingBase.playSound(SoundEvents.ENTITY_GENERIC_DRINK, 0.5F,
                            livingBase.world.rand.nextFloat() * 0.1F + 0.9F);
                }
            }
        }
    }
    ...
    public static void onAdvancement(AdvancementEvent event) {
        if (GTFOConfig.gtfoOtherFoodModConfig.reduceForeignFoodStats) {
            ...
            if (divisorObtained > 1 &&
                    GTFOAppleCoreCompat.advancementLookup(event.getEntityPlayer()) == divisorObtained) {
    ...
    public static void onDrinkPotion(PotionEvent.PotionAddedEvent event) {
        if (PotionAmplifierPotion.INSTANCE != null &&
                event.getEntityLiving().isPotionActive(PotionAmplifierPotion.INSTANCE) &&
                !event.getPotionEffect().getPotion().equals(PotionAmplifierPotion.INSTANCE)) {
            int amplificationBonus = event.getEntityLiving().getActivePotionEffect(PotionAmplifierPotion.INSTANCE)
                    .getAmplifier() + 1;
        ...
        if (PotionLengthenerPotion.INSTANCE != null &&
                event.getEntityLiving().isPotionActive(PotionLengthenerPotion.INSTANCE) &&
                !event.getPotionEffect().getPotion().equals(PotionLengthenerPotion.INSTANCE)) {
            int durationBonus = event.getEntityLiving().getActivePotionEffect(PotionLengthenerPotion.INSTANCE)
                    .getAmplifier();
            Potion type = event.getPotionEffect().getPotion();
    ...
    public static void handleBerryGrowth(BlockEvent.CropGrowEvent.Pre event) {
        if (event.getState().getBlock() instanceof GTFOBerryBush bush) {
            event.setResult(
                    GTValues.RNG.nextInt(bush.getGrowthSlowdown(event.getWorld(), event.getPos(), event.getState())) ==
                            0 ? Event.Result.ALLOW : Event.Result.DENY);
        }
    }
